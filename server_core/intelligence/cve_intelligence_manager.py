...
"""
    ...
    # ═══════════════════════════════════════════════════════════════════
    # NEW METHODS — HexStrike merger enrichment
    # ═══════════════════════════════════════════════════════════════════

    def _search_exploits_internal(self, cve_id: str) -> Dict[str, Any]:
        """Internal stub: search for exploits (overridden by real DB lookups)."""
        return {"success": True, "exploits": []}

    def _get_cve_data_internal(self, cve_id: str) -> Dict[str, Any]:
        """Internal stub: get CVE data from cache or feed."""
        return self.cve_cache.get(cve_id, {"description": "", "cvss_score": 5.0})

    def exploitability_analysis(self, cve_id: str) -> Dict[str, Any]:
        """Analyze whether a CVE has known public exploits and assess maturity.

        Returns:
            Dict with exploitable, exploit_maturity, public_exploits count,
            metasploit_module, exploitdb_entries, ease_of_exploitation.
        """
        logger.info("Analyzing exploitability for %s", cve_id)
        result = {
            "cve_id": cve_id, "exploitable": False,
            "exploit_maturity": "unproven", "public_exploits": 0,
            "metasploit_module": False, "exploitdb_entries": [],
            "github_pocs": 0, "ease_of_exploitation": "unknown",
        }
        try:
            exploits = self._search_exploits_internal(cve_id)
            if exploits.get("success") and exploits.get("exploits"):
                exp_list = exploits["exploits"]
                result["public_exploits"] = len(exp_list)
                for exp in exp_list:
                    src = exp.get("source", "").lower()
                    if src == "metasploit":
                        result["metasploit_module"] = True
                    elif src == "exploit-db":
                        result["exploitdb_entries"].append(exp.get("title", ""))
                if result["metasploit_module"]:
                    result["exploit_maturity"] = "high"
                elif result["public_exploits"] >= 3:
                    result["exploit_maturity"] = "functional"
                elif result["public_exploits"] >= 1:
                    result["exploit_maturity"] = "poc"
                result["exploitable"] = result["public_exploits"] > 0
            cve_data = self._get_cve_data_internal(cve_id) or {}
            cvss = cve_data.get("cvss_score", 0)
            if result["exploit_maturity"] == "high" and cvss >= 7.0:
                result["ease_of_exploitation"] = "easy"
            elif result["exploit_maturity"] in ("functional", "poc"):
                result["ease_of_exploitation"] = "moderate"
            elif cvss >= 9.0:
                result["ease_of_exploitation"] = "possible"
            else:
                result["ease_of_exploitation"] = "difficult"
        except Exception as e:
            logger.error("Exploitability analysis failed: %s", e)
            result["error"] = str(e)
        return result

    def correlate_with_threat_feeds(self, cve_ids):
        """Cross-reference CVEs with known threat actor campaigns.

        Returns dict mapping each CVE to threat_actors, campaigns, ransomware_use.
        """
        logger.info("Correlating %d CVEs with threat feeds", len(cve_ids))
        known = {
            "CVE-2021-44228": {"threat_actors": ["APT41","Hafnium","Sandworm"],
                "campaigns": ["Log4Shell"], "ransomware_use": True},
            "CVE-2021-26855": {"threat_actors": ["Hafnium"],
                "campaigns": ["ProxyLogon Exchange"], "ransomware_use": True},
            "CVE-2023-34362": {"threat_actors": ["Clop"],
                "campaigns": ["MOVEit Transfer"], "ransomware_use": True},
        }
        results = {}
        for cid in cve_ids:
            results[cid] = known.get(cid, {"threat_actors": [],
                "campaigns": [], "ransomware_use": False})
        return results

    def zero_day_research_workflow(self, software: str, version: str) -> Dict[str, Any]:
        """Analyze software/version for potential zero-day research targets.

        Returns dict with surface_area_score, potential_vulns, fuzzing targets.
        """
        logger.info("Zero-day research for %s %s", software, version)
        sw_lower = software.lower()
        result = {"software": software, "version": version,
            "surface_area_score": 0.5, "potential_vulns": [], "recommended_fuzzing_targets": []}
        if any(k in sw_lower for k in ["apache","nginx","iis","tomcat"]):
            result["surface_area_score"] = 0.9
            result["recommended_fuzzing_targets"] = ["HTTP parsing","Header parsing","URI normalisation","SSL/TLS handshake"]
        elif any(k in sw_lower for k in ["wordpress","drupal","joomla"]):
            result["surface_area_score"] = 0.85
            result["recommended_fuzzing_targets"] = ["REST API endpoints","File upload handlers","Plugin installation","XML-RPC"]
        elif any(k in sw_lower for k in ["mysql","postgresql","mongo"]):
            result["surface_area_score"] = 0.7
            result["recommended_fuzzing_targets"] = ["Query parser","Auth handshake","Replication protocol"]
        else:
            result["recommended_fuzzing_targets"] = ["Input parsing","File format parsers","Network protocol handling"]
        result["potential_vulns"] = [
            {"type":"memory_corruption","likelihood":"medium","targets":["Input buffers","String operations"]},
            {"type":"injection","likelihood":"high","targets":["User-supplied parameters","File paths"]},
            {"type":"auth_bypass","likelihood":"medium","targets":["Session management","Token validation"]},
            {"type":"info_disclosure","likelihood":"high","targets":["Error messages","Debug endpoints"]},
        ]
        return result

    def generate_attacker_perspective(self, cve_id: str) -> str:
        """Generate a red-team narrative describing how an attacker would exploit this CVE."""
        cve_data = self._get_cve_data_internal(cve_id) or {}
        ...
        return f"""# Attacker Perspective: {cve_id}

## Overview
{desc[:300]}

## Attack Flow
1. **Recon**: Identify targets running vulnerable version.
2. **Weaponize**: {'Use public exploit or Metasploit module.' if cvss >= 7.0 else 'Develop custom exploit.'}
3. **Deliver**: Send crafted payload to vulnerable endpoint.
4. **Exploit**: Trigger vulnerability for {'RCE' if cvss >= 8.0 else 'unauthorized access'}.
5. **Post-Exploit**: {'Establish persistence, escalate privileges.' if cvss >= 7.0 else 'Limited post-exploit.'}

## Defensive Actions
1. Apply vendor patch immediately.
2. Implement WAF virtual patching.
3. Monitor for exploitation IOCs.

Severity: **{severity}** | CVSS: **{cvss}**
"""

    def calculate_risk_score(self, cve_id: str) -> Dict[str, Any]:
        """Calculate combined risk score (0-100) factoring CVSS, exploits, and threat intel."""
        logger.info("Calculating risk score for %s", cve_id)
        cvss_contrib = 17.5; exploit_contrib = 5; threat_contrib = 0; complexity_contrib = 5
        try:
            cve_data = self._get_cve_data_internal(cve_id) or {}
            cvss = cve_data.get("cvss_score", 5.0)
            cvss_contrib = min(cvss * 3.5, 35)
            exploitability = self.exploitability_analysis(cve_id)
            maturity_pts = {"high":35,"functional":25,"poc":15,"unproven":5}
            exploit_contrib = maturity_pts.get(exploitability.get("exploit_maturity","unproven"), 5)
            correlation = self.correlate_with_threat_feeds([cve_id])
            cve_corr = correlation.get(cve_id, {})
            threat_contrib = 0
            if cve_corr.get("ransomware_use"):
                threat_contrib += 10
            if cve_corr.get("threat_actors"):
                threat_contrib += min(len(cve_corr["threat_actors"])*2, 5)
            ease = exploitability.get("ease_of_exploitation","difficult")
            ease_pts = {"easy":15,"moderate":10,"possible":5,"difficult":2}
            complexity_contrib = ease_pts.get(ease, 2)
            overall = min(cvss_contrib + exploit_contrib + threat_contrib + complexity_contrib, 100)
            if overall >= 80:
                rec = "IMMEDIATE ACTION: Patch within 24 hours."
            elif overall >= 60:
                rec = "HIGH PRIORITY: Patch within 1 week."
            elif overall >= 40:
                rec = "MODERATE: Patch in next maintenance window."
            else:
                rec = "LOW PRIORITY: Patch on regular schedule."
        except Exception as e:
            logger.error("Risk score failed: %s", e)
            overall = 50; rec = "Manual review required."
        return {"cve_id": cve_id, "overall_risk": round(overall,1),
            "risk_level": "CRITICAL" if overall>=80 else "HIGH" if overall>=60 else "MEDIUM" if overall>=40 else "LOW",
            "factors": {"cvss_score": round(cvss_contrib,1), "exploit_availability": exploit_contrib,
                "threat_intelligence": threat_contrib, "attack_complexity": complexity_contrib},
            "recommendation": rec}
